...
from helpers import group_zero_cover_percentage_data
...
@pytest.mark.asyncio
async def test_zero_covers_percentage_data(start_date, end_date, get_worksheet_excel_data):
    async with async_playwright() as playwright:
        ...
        group_takeaway_cover_grouped_data = await group_zero_cover_percentage_data(ws_data, day_range, month_range, year_range)

        for values in group_takeaway_cover_grouped_data:
            restaurant_name = values
            checks_with_covers = group_takeaway_cover_grouped_data[values]["with_covers"]
            checks_with_zero_covers = group_takeaway_cover_grouped_data[values]["with_zero_covers"]
            total_checks = group_takeaway_cover_grouped_data[values]["total_checks"]
            zero_covers_percentage = group_takeaway_cover_grouped_data[values]["zero_covers_percentage"] * 100
